    ...
    <div class="row px-xl-5">
        ...
        <div class="col-lg-7 pb-5">
            ...
            <div class="d-flex mb-3">
                ...
                <small class="pt-1">({{ count($productComments) }} {{ __('Nhập xét') }})</small>
            </div>
                ...
                <div class="d-flex align-items-center mb-4 pt-2">
                    ...
                    @if ($product_detail->qty <= 0)
                        <button type="button" disabled class="btn btn-primary px-3"><i
                            class="fa fa-ban mr-1"></i> {{ __('Sản phẩm đã hết hàng') }}</button>
                        @else
                        <button onclick="unPayNow()" type="submit" class="btn btn-primary px-3"><i
                                class="fa fa-shopping-cart mr-1"></i> {{ __('Thêm vào giỏ') }}</button>
                        <input id="pay_now" type="hidden" name="pay_now" value="false">
                        <button onclick="payNow()" type="submit" class="btn btn-outline-primary px-3 ml-3">{{ __('Mua ngay') }}</button>
                        {{-- </div> --}}
                        @endif
                </div>
            ...
            <div class="d-flex pt-2">
                <p class="text-dark font-weight-medium mb-0 mr-2">{{ __('Share on:') }}</p>
            ...
            <div class="">{{ __('Còn:') }} {{ $product_detail->qty }} </div>
        </div>
    </div>
    <div class="row px-xl-5">
        <div class="col">
            <div class="nav nav-tabs justify-content-center border-secondary mb-4">
                <a class="nav-item nav-link active" data-toggle="tab" href="#tab-pane-1">{{ __('Mô tả') }}</a>
                <a class="nav-item nav-link" data-toggle="tab" href="#tab-pane-3">{{ __('Đánh giá') }}
                    ({{ count($productComments) }})</a>
            </div>
            <div class="tab-content">
                <div class="tab-pane fade show active" id="tab-pane-1">
                    <h4 class="mb-3">{{ __('Mô tả sản phẩm') }}</h4>
                    {!! $product_detail->content !!}
                </div>
                <div class="tab-pane fade" id="tab-pane-3">
                    <div class="row">
                        <div class="col-md-6">
                            <h4 class="mb-4">{{ count($productComments) }} {{ __('Đánh giá cho') }} "{{ $product_detail->name }}"</h4>
                        ...
                        @if (Auth::check())
                        <div class="col-md-6">
                            <h4 class="mb-4">{{ __('Để lại đánh giá') }}</h4>
                            <form method="POST">
                                @csrf
                                <input type="hidden" name="user_id" value="{{Auth::user()->id}}">
                                <input type="hidden" name="email" value="{{ Auth::user()->email }}">
                                <div class="d-flex my-3">
                                    <p class="mb-0 mr-2">{{ __('Đánh giá sao *') }}</p>
                                    <div class="rate">
                        ...
                        <div class="col-md-6">
                            ...
                            <a href="{{ route('loginUser') }}?redirect_uri={{url()->full()}}"> {{ __('Đăng nhập để đánh giá') }}</a>
                        </div>
                        @endif
                    </div>
                </div>
...
<div class="container-fluid py-5">
    <div class="text-center mb-4">
        <h2 class="section-title px-5"><span class="px-2">{{ __('Sản phẩm liên quan') }}</span></h2>
    </div>
    <div class="row px-xl-5">
        <div class="col">
                ...
                <div class="card product-item border-0">
                    ...
                    <div class="card-footer d-flex justify-content-between bg-light border">
                        <a href="{{ route('detail', ['id' => $item->id, 'slug' => $item->slug]) }} "
                            class="btn btn-sm text-dark p-0"><i class="fas fa-eye text-primary mr-1"></i>{{ __('Xem chi tiết') }}</a>
